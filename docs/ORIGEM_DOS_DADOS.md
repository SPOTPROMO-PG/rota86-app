...
| Leitura (`leitura_por_loja.csv`) + STAR (`lojs_star.csv`) — **dois arquivos separados na origem** (confirmado 16/09/2026: leitura tem ~2.937 lojas, STAR só ~24 — a maioria nunca entra na lista de STAR, o que não é pendência, é ausência de registro) | Unidos por `ID_LOJA` (LEFT JOIN a partir da leitura) pelo motor `unir_leitura_star.py`, que gera `retailx_leituras.csv` | `rota86.stg_leitura_star_loja` (a partir do arquivo já unificado) | `rota86.leitura_star_loja_mes` — `status_star` fica `NULL` (não `NOK`) quando a loja não está na lista de STAR daquela competência |
